...
const { connect, pageController } = require('../lib/cjs/index.js');
const BitBrowserAPI = require('../lib/cjs/module/bitBrowserAPI.js');
const { BitBrowserLogger } = require('../lib/cjs/module/bitBrowserHelper.js');
const fs = require('fs');
...
// 辅助函数：在页面内随机移动鼠标
async function moveMouseRandomly(page, options = {}) {
  const { moveDelay = 1000, randomOffset = 20 } = options;
  ...
  try {
    ...
    logger.warn(`Mouse move failed: ${error.message}`);
  }
}
